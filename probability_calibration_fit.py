...
if __name__ == "__main__":
    # sys.exit 로 감싼다(2026-09-23 수정). main() 이 실패 경로에서 1 을
    # 돌려주는데 그냥 호출하면 그 값이 버려져 **종료코드가 0** 이 된다 —
    # `--patch-metrics` 의 "재계산 결과가 저장값과 다르면 멈춘다"는
    # 안전장치가 통째로 무력화돼 있었다. 이 저장소가 반복해 겪은
    # "게이트는 실패를 알렸는데 호출부는 초록불" 유형이다.
    sys.exit(main())
