...
(표형 데이터에서 부스팅 트리의 우위), 한파에서 특히 크다.
    ...
    폭염 F1 0.7987 → 0.8345 · 한파 0.4607 → **0.6077** · 황사 0.1410 → 0.1699

한파 +0.147 은 이 저장소가 그 헤드에 쏟은 모든 시도(중첩 전이학습·헤드
디커플링·커리큘럼·GroupDRO)를 합친 것보다 크다.

**위 값은 실험 채점이다** — 판정선을 검증셋의 보정용 절반에서 고르고 평가용
절반에서만 채점하는 `baseline_suite.honest_f1` 규약을 두 모델에 같이 적용한
것이라 서로 비교할 수 있다. 화면과 README 의 배포 표는 **서빙 채점**(화면이
쓰는 판정선을 공식 라벨 전체에 적용)이라 값이 다르다(한파 0.552 → 0.654).
**두 채점을 섞어 인용하지 말 것.** 이 모듈이 모델 파일에 적는
`meta_<head>_f1_cal` 이 위 실험 채점값이고, 배포 표의 값은
`metrics_report.py --patch-checkpoint` 가 적는다.
...
와 무관하게 단조성이 성립한다. 실측(`coldwave_pathway_check.py`, 12개 기준
레코드 × 12개월 × 11개 기온): 기온이 오를 때 한파 확률이 오른 인접 쌍이
**0/1,440**이다. 폭염(부호 반대)도 0/1,440 이며 두 리드타임 모두 그렇다.

**단조성 판정에 순위상관을 쓰지 말 것.** `+6h` 한파의 최악 상관은 −0.9273
이지만 `+12h` 는 **+0.1090**(겉보기 역전)으로 나오는데, 같은 격자의 위반이
0/1,440 이다. `+12h` 한파의 보정이 등온 회귀라 확률을 평탄 구간으로 접고,
격자 대부분이 한 값으로 접히면 남은 미세한 차이가 순위를 지배한다(보정 전
상관 −0.942). `coldwave_pathway_check.violations()` 가 판정하고 상관은
참고값으로만 출력한다.
