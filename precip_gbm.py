...
0.5933 · 강수 구간 MAE 2.0471 vs 2.1014 — `baseline_suite.py` 의 탐색용
GBM 을 **평가용 절반**에서 잰 값이며, 두 모델에 같은 규약을 적용했으므로
서로 비교할 수 있다). 원인을 갈라 보니 우리가 할 수 있는
...
**세 가지 채점이 섞이지 않게 주의할 것.** ① 위 비교는 평가용 절반(탐색용
GBM). ② 내보낸 모델 자신의 값은 npz 의 `meta_val_precip_*` 이고 역시 평가용
절반이다(F1 0.6397 · 구간 MAE 2.0467 · 서빙 MAE 0.1702). ③ 화면과 README 가
쓰는 배포 값은 **검증셋 전체**에서 `metrics_report.py --patch-checkpoint` 가
적는다(F1 0.6276 · 구간 MAE 2.0987 · 서빙 MAE 0.1731). 표본이 다르므로 한
문장 안에서 섞어 인용하면 안 된다.
