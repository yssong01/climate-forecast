...
st.sidebar.caption(
    ...
    "학습시키는 게 아니라, 그 값으로 추론만 새로 수행합니다."
)
st.sidebar.caption(
    "단, 관측은 매시 정각에 한 번뿐이고 약 10분 뒤 공개됩니다. 그래서 같은 정시 "
    "안에서 다시 실행하면 새로 조회하지 않고 직전 결과를 재사용합니다 — 갱신 주기를 "
...
with tab_trend:
    ...
    if len(series) < 2:
        ...
        st.caption(
            f"선과 막대는 실제 관측값입니다 — 최근 72시간 중 관측이 있는 "
...
with tab_extreme:
    ...
    if not any(ev.get(k) is not None for k in ("heatwave", "coldwave", "dust")):
        ...
        st.caption(
            ...
            "사건으로 친다\"는 판정선입니다. 대부분의 시각엔 사건이 아니므로 "
            "확률이 낮게 나오는 게 정상입니다 — 실제 폭염·한파가 다가올 때만 "
            "이 값이 올라 판정선을 넘습니다."
        )
        ...
        st.caption(
            ...
            "(황사라고 판정한 것 중 88.3%가 오탐)에 그쳐 실용 수준이 못 됩니다. "
            "PM10 농도는 국외(주로 중국) 장거리 수송이 지배하는데, 이 모델의 "
            "입력은 국내 12개 지점의 기온·습도·기압·풍속뿐이라 원리적으로 "
            "구분할 단서가 부족합니다. 헤드 자체는 남아 있고 값도 계산되지만, "
            "화면에 내놓을 만큼 믿을 수 없어 가리기로 했습니다."
        )
        ...
        st.caption(
            ...
            "한파 −12°C 이하)으로 근사해 채웠습니다. 그런데 공식 특보는 하루 "
            "단위로 발효되는 반면 근사는 그 시각 기온에만 반응해, 같은 현상에 "
            "정의가 다른 두 라벨이 섞였습니다. 공식 기록이 있는 표본만 쓰도록 "
            "바꿨습니다(2026-08-16 재학습)."
        )
        st.caption(
...
with tab_perf:
    ...
    if val_temp is not None:
        ...
        st.caption(
            ...
            "판단해야 의미가 생깁니다. 위 기준선 값은 모델과 똑같은 검증 표본에서 "
        ...
        if _naive_precip and val_precip > _naive_precip:
            ...
            st.warning(
                ...
                f"표현을 두고 경쟁해 무강수 억제력이 약해졌습니다(초기 −25.6%). 가중치 하향으로 "
                f"현재 수준까지 좁혔습니다. 이후 학습 데이터를 3.6년→13.6년으로 늘려도 이 격차는 "
                f"더 좁혀지지 않았습니다 — 데이터양이 아니라 헤드 간 경쟁 구조 자체가 원인이라는 "
                f"뜻입니다. 강수는 MAE가 아니라 '비가 올지 안 올지 맞혔나'로 보는 것이 실용적이며, "
                f"그 지표는 아래 '출력값 적중률'의 강수 항목입니다.",
                icon="⚠️",
            )
    ...
    if _em:
        ...
        st.caption(
            ...
            "95% 신뢰구간(Wilson score)입니다. 한파·폭염은 며칠씩 이어지는 사건이라 "
            "같은 사건 안의 연속 시간 표본은 서로 강하게 연관돼 있습니다(독립이 아님) "
            "— 그래서 '24시간 이내로 이어지는 표본을 사건 하나로 묶어 센 수'를 유효 "
            "표본 크기로 씁니다. 사건이 적을수록 십자가 커집니다(신뢰도가 낮다는 뜻을 "
            "그대로 보여줍니다). 정확도(accuracy) 대신 재현율·정밀도를 쓴 이유는 위와 "
            "같습니다(양성 희박)."
        )
        ...
        if os.path.exists(_calib_path):
            ...
            st.caption(
                "폭염은 부산(159)만 확 처지는데(재현율 71%·정밀도 47%) 원인을 "
                "확인했습니다 — 모델이 폭염이라 잘못 예측한 날의 실제 기온이 "
                "28.65°C로, 부산 자신의 진짜 폭염일 평균(27.38°C)보다 오히려 "
                "높습니다. 부산만 공식 기준이 다른데 모델은 관측소 구분 없이 "
                "공통 기준으로 배웠다는 뜻입니다. 한파는 대구·강릉의 재현율이 "
                "유독 낮은 이유도 확인했습니다 — 시간 표본은 216~384건으로 적지 "
                "않아 보였지만, 사건 단위로 세면 각각 10개·8개뿐이었습니다"
                "(춘천은 같은 기준 40개) — 표본 부족이 맞았습니다."
            )
        else:
            st.caption(
    ...
    st.caption(
        ...
        "보정용 / 평가용으로 무작위 절반씩 나눕니다(학습·검증을 나눌 때와는 다른 "
        "난수를 씁니다). ② 보정용에서만 F1이 가장 높아지는 값을 찾습니다. ③ 그 값을 "
        "한 번도 보지 않은 평가용에 적용해 성능이 유지되는지 확인합니다."
    )
    st.caption(
        "이 절차가 필요한 이유: 같은 데이터에서 값을 고르고 그 데이터로 채점하면 "
        "그 데이터에만 맞춘 값이 좋아 보이게 됩니다(과적합). 실제로 강수·황사는 "
        "이 검증을 통과해 새 값을 채택했고, 폭염·한파는 통과하지 못해(개선폭이 "
        "0.01에 못 미침) 기본값 0.5를 유지했습니다."
    )
...
with tab_model:
    ...
    if gw:
        ...
        st.caption(
            "세 축의 기여도는 고정값이 아니라 값을 낼 때마다 입력을 보고 다시 계산됩니다. "
    ...
    st.markdown(
        "이 모델은 값을 하나 낼 때 서로 다른 세 가지 질문을 던지고, 그 답 셋을 "
    ...
    st.markdown(
        ...
        "이번 시각 이 관측소의 관측값 그대로 — 기온·강수·습도·풍속·풍향·기압· "
        "위도·경도 등 숫자 12개 |\n"
    ...
    st.caption(
        ...
        "Re는 실수부(Real), Im은 허수부(Imaginary)에서 온 말입니다. 복소수에서 "
        "이 둘은 서로 직각 방향이라, \"세 축이 서로 겹치지 않는 정보를 담는다\"는 "
    ...
    st.caption(
        ...
        "없는 정보입니다. 그래서 축을 추가한 만큼 실제로 새 정보가 들어옵니다."
    )
    st.caption(
        ...
        "확인하고 그 데이터가 아무 정보도 주지 않았다는 결론을 냈습니다. 이후 "
    ...
    st.caption(
        "세 축은 각각 숫자 여러 개가 늘어선 벡터로 바뀐 뒤 아래 식으로 합쳐집니다. "
    ...
    with st.expander("이 식은 어디서 왔나? — 원 논문과의 관계"):
        ...
        st.markdown(
            ...
            "상대적 비중입니다. 위쪽 식은 셋을 대등하게 `w`로 적었을 뿐 같은 식이며, "
    ...
    st.markdown(
        f"위에서 만든 숫자 {_embed_dim}개짜리 목록 `s`는 여기까지가 공통 과정입니다. "
        ...
        "- 폭염 · 한파 · 황사 — 각각 \"일어날 확률\" 하나씩. 폭염·한파는 "
        "'극한 기상' 탭의 막대가 이 값이고, 황사는 신뢰도가 낮아 화면에서 뺐습니다."
    )
    st.caption(
        "기온·강수처럼 숫자 값을 맞히는 것을 회귀(regression), 폭염·한파·황사처럼 "
        "일어난다/아니다를 맞히는 것을 이진분류(binary classification)라고 부릅니다. "
    ...
    st.markdown(
        ...
        "각각 의미 있는 범위로 바꿔줍니다.\n"
        "- σ(시그마 / 시그모이드) — 어떤 실수든 0~1 사이로 눌러 담는 함수. "
