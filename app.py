...
with tab_trend:
    ...
    if result_12h is not None:
        ...
        # 가장 크게 무너지는 것은 강수다.
        # 2026-09-07 정정 — +6시간만 수치예보(NWP) 입력을 쓰는 모델로 교체됐고
        # +12시간은 아직 종전 체크포인트다. 두 값을 나란히 놓을 때 그 사실을
        # 밝히지 않으면, 리드타임 차이로 보이는 것 중 일부가 실은 **모델 세대
        # 차이**라는 점이 가려진다.
        st.caption(
            "+12시간은 2차 산출값이다 — 절대오차가 +6시간보다 크다(기온 MAE "
            "1.17→1.66°C, 검증셋 평균). 다만 지금 두 값은 **세대가 다른 두 "
            "모델의 출력**이다: +6시간은 2026-09-07에 수치예보(NWP) 예보값을 "
            "입력으로 편입한 모델로 교체됐고, +12시간은 그 이전 체크포인트를 "
            "그대로 쓴다. 따라서 아래 격차에는 리드타임 효과와 모델 차이가 "
            "함께 섞여 있으며, 리드타임만의 효과로 읽어서는 안 된다. "
            "+12시간 자체의 값은 강수 기준선 대비 −25.1%, 폭염 F1 0.776, "
            "한파 0.509, 황사 0.192이고 계절 오탐 1.21%로 차단 기준(10%) "
            "아래다. '성능 검증' 탭 수치는 +6시간 기준이다."
        )
