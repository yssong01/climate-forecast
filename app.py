...
with tab_extreme:
    ...
    if not any(ev.get(k) is not None for k in ("heatwave", "coldwave", "dust")):
        ...
        st.caption(
            ...
            "의미로 읽어도 된다. 보정 전 원본 출력은 체계적으로 과신했다(한파 "
            "60~70%로 표시되던 구간의 실제 빈도가 27%). 희소 사건이 '항상 아님'"
            "으로 붕괴하는 것을 막으려고 학습 시 양성 표본에 가중치를 준 것이 "
            "확률을 위로 밀어올린 결과이며, 검증셋에서 등온 회귀(isotonic "
            "regression)로 되돌렸다. 보정은 순위를 바꾸지 않는 단조 변환이므로 "
            "판정 성능(F1)은 그대로다. 상세 내용은 '성능 검증' 탭을 참조한다."
        )
        ...
        st.caption(
            ...
            "유의하지 않아(순이득 각각 0.005·0.009 — 채택 기준 0.01 미달) 원본 "
            "확률 기준 0.5를 유지했다. 단, 부산은 예외다 — 관측소별로 분해하면 "
            "부산 폭염만 정밀도가 유의하게 낮았고(아래 '관측소별 성능' 참조), 부산만 "
            "별도로 재보정한 결과 순이득 +0.039로 유의하여 원본 기준 0.33으로 "
            "낮췄다. 위에 표시되는 값이 이보다 낮은 것은 확률 보정을 거치면서 "
            "임계값도 같은 곡선으로 함께 옮겼기 때문이며, 판정 결과는 동일하다."
        )
        st.caption(
...
with tab_perf:
    ...
    if _cal:
        ...
        st.caption(
            ...
            "모델이 '항상 아님'으로 답하는 자명해로 붕괴하기 쉬워, 학습 시 양성 "
        ...
        st.caption(
            ...
            "개선을 확인한 헤드만 채택했다(임계값 산출과 동일한 절차). 순위를 "
            "바꾸지 않는 단조 변환이므로 판정 성능은 보존된다 — 임계값을 같은 "
            "곡선으로 함께 옮기기 때문이다."
        )
        _rows = ["| 사건 | 보정 전 ECE | 보정 후 ECE | F1 변화 |", "|---|---|---|---|"]
        for _k, _ko, _e0, _e1, _f in (
            ("heatwave", "🔥 폭염", 0.0388, 0.0043, "0.8122 → 0.8119"),
            ("coldwave", "🥶 한파", 0.0446, 0.0034, "0.4629 → 0.4629"),
        ):
            if _k in _cal:
