...
with tab_extreme:
    ...
    if not any(ev.get(k) is not None for k in ("heatwave", "coldwave", "dust")):
        ...
        st.caption(
            "⚠️ 황사 확률은 이 화면에서 뺐습니다. 재보정 후에도 정밀도가 0.134"
            "(황사라고 판정한 것 중 86.6%가 오탐)에 그쳐 실용 수준이 못 됩니다. "
        ...
        st.caption(
            ...
            "0.5로 자르면 균형이 맞지 않을 수 있습니다. 그래서 검증 데이터를 둘로 "
            "갈라 한쪽에서 F1이 가장 높아지는 값을 찾고, 한 번도 보지 않은 다른 "
            "쪽에서도 성능이 유지되는지 확인합니다(자세한 절차는 '성능 검증' 탭). "
            "폭염·한파는 이 재보정으로 얻는 이득이 통계적으로 유의하지 않아(순이득 "
            "각각 0.007·0.001) 기본값 0.5를 그대로 씁니다 — 재보정한다고 항상 더 "
            "나아지는 건 아닙니다. 한파는 이전 체크포인트에서는 재보정(임계값 "
            "0.89)이 유의미했는데, 헤드를 다시 미세조정하면서 확률 분포가 바뀌어 "
            "그 근거가 사라졌습니다(2026-08-16)."
        )
        st.caption(
...
with tab_perf:
    ...
    st.caption(
        ...
        "그 데이터에만 맞춘 값이 좋아 보이게 됩니다(과적합). 실제로 폭염 판정선 "
        "재보정은 이 검증을 통과하지 못해(개선폭이 0.01에 못 미침) 기본값 0.5를 "
        "유지했고, 강수·한파는 통과해 새 값을 채택했습니다."
    )
