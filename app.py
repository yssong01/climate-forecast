...
with tab_extreme:
    ...
    if not any(ev.get(k) is not None for k in ("heatwave", "coldwave", "dust")):
        ...
        st.caption(
            ...
            "널 캘리브레이션(무사건 구간 확률의 μ+z·σ를 임계값 후보로 역산하는 "
            "방식)으로 8건이 추가 채택됐다 — 폭염 인천·수원·청주, 한파 강릉·서울·"
            "인천·수원·광주(순이득 +0.016~+0.092). 부산 폭염에서는 이 방식이 기존 "
            "F1 그리드서치와 F1이 완전히 일치(0.5735)해 방법의 타당성이 뒷받침됐다. "
