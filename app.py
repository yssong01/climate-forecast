...
import accuracy
# 특징 집합 정의만 읽는다(순수 상수·함수 — 파일 조회 없음). '모델 구조' 탭이
# Z축 차원을 상수로 적어두면 입력이 늘어도 화면만 옛 값에 머문다 — 실제로
# 수치예보 14차원이 편입된 뒤에도 "수치 14개"로 남아 있었다(2026-09-23 발견).
from nwp_collector import feature_dim as nwp_feature_dim
...
def extreme_metrics_of(ckpt: dict) -> tuple[dict, bool]:
    """극한기상 분류 지표와 "서빙 판정선 기준인가" 여부.

    두 탭이 각자 체크포인트를 뒤져 서로 다른 키를 읽던 것을 한 곳으로
    모은다(2026-09-23 점검에서 발견). '극한 기상' 탭은 `extreme_metrics`
    (학습이 t=0.5 로 잰 값), '성능 검증' 탭은 `extreme_metrics_served`
    (서빙 판정선 기준)를 읽고 있어, **같은 황사 정밀도가 한 탭에서는
    7.2%, 다른 탭에서는 14.0% 로 표시**됐다. 게다가 후자의 캡션은 그 값을
    "원본 확률 0.5 기준"이라고 잘못 밝히고 있었다.

    화면에 나가는 판정과 표의 값이 어긋나지 않아야 하므로 서빙 기준을
    우선하고, 없으면(구버전 체크포인트) t=0.5 값으로 내려가되 어느 쪽인지를
    호출부가 캡션에 밝힐 수 있게 함께 돌려준다.
    """
    served = ckpt.get("extreme_metrics_served") or {}
    if served:
        return served, True
...
try:
    ...
    def _gate(_gbm=None, _lead=6):
        """강수 판정선 — 강수를 GBM 이 내면 **그 모델 파일의 값**을 쓴다.

        `_thr()` 과 같은 이유로 둔다(2026-09-26). 서빙은 이미
        `predict.py` 에서 `meta_gate_tau` 를 쓰고 있었는데, **화면 서술과
        근접 판정 표시만 신경망 상수(`PRECIP_PROB_GATE_BY_LEAD`, +6h
        0.85)를 계속 읽고 있었다** — 실제 판정선은 0.310 이라 화면이
        "85% 미만이면 0으로 처리한다"고 설명하는 동안 모델은 31% 를
        기준으로 자르고 있었다. 근접 판정 안내(`_gate - 0.10`)도 그만큼
        엉뚱한 구간을 가리켰다. 값을 아는 주체가 둘이면 어긋난다.
        """
        g = precip_gbm if _gbm is None else _gbm
        if g is not None:
            return float(g[2]["meta_gate_tau"])
        return PRECIP_PROB_GATE_BY_LEAD.get(_lead, PRECIP_PROB_GATE)
...
with tab_trend:
    ...
    # 물들인다.
    result_12h = None
    # 아래 try 안에서 예외가 나면 이 이름들이 미정의로 남아, 그 뒤 화면
    # 코드가 NameError 로 죽는다 — 판정선 서술이 이 값을 읽는다(2026-09-26).
    gbm_12h = egbm_12h = None
    try:
    ...
    def _precip_err(_ck, _val):
        ...
        ② 검증셋의 93.4%가 무강수라 그 평균은 건조 표본이 지배한다 — 강수가
    ...
    def _rain_prob_cell(_f, _gate):
        ...
        **이 값은 보정 전 원본 확률이다.** 극한기상 세 헤드와 달리 강수
        헤드는 보정 곡선을 서빙에 적용하지 않으므로(곱셈 구조가 보정 전
        확률과 맞물려 학습됐다 — `predict.py` 강수 게이팅 주석), 판정선도
        같은 원본 공간의 값이다. 화면이 이 사실을 밝히지 않으면 '극한 기상'
        탭의 "표시된 확률은 보정된 값"이라는 설명이 여기까지 적용되는
        것으로 읽힌다(2026-09-23 점검에서 발견).
        """
    ...
    _rows = [
        ...
        {
            "항목": "강수 확률",
            "+6시간(기본)": _rain_prob_cell(f6, _gate()),
        },
    ]
    ...
    st.caption(
        ...
        f"설명). 강수 확률이 +6시간은 {_gate():.0%}, "
        f"+12시간은 {_gate(gbm_12h, 12):.0%} "
        "미만이면 강수량을 0으로 반올림한다(리드타임마다 확률 분포가 달라 "
        "임계값을 따로 골랐고, 강수를 전용 GBM 이 내는 구성에서는 그 모델 "
        "파일에 저장된 값을 쓴다). "
        "표의 강수 확률과 이 임계값은 **확률 보정을 거치지 않은 원본 값**이다 "
        "— 강수 헤드는 보정 곡선을 서빙에 적용하지 않으며, 보정을 적용하는 "
        "것은 '극한 기상' 탭의 폭염·한파 확률이다('성능 검증' 탭의 확률 보정 "
        "절 참조). "
        "'±' 뒤 숫자는 "
    ...
    for _lbl, _res, _f, _g in (
        ("+6시간", result, f6, _gate()),
        ("+12시간", result_12h, f12 if result_12h is not None else None,
         _gate(gbm_12h, 12) if result_12h is not None else None),
    ):
        if _res is None or _f is None:
            continue
        _rp = _f.get("rain_prob")
        if _rp is not None and _f["precipitation"] == 0.0 and _rp >= _g - 0.10:
            st.caption(
                f"ℹ️ {_lbl} 강수확률은 {_rp:.0%}로 판정 임계값({_g:.0%})에 "
...
with tab_extreme:
    ...
    if not any(ev.get(k) is not None for k in ("heatwave", "coldwave", "dust")):
        ...
        _ex_gbm_on = ckpt.get("extreme_source") is not None
        st.caption(
            "이 값은 실제 발생 빈도에 맞춰 보정(calibration)한 확률이다 — "
            "\"30%\"로 표시된 상황들을 모으면 실제로 약 30%에서 사건이 발생했다는 "
            "의미로 해석할 수 있다. 판정 임계값도 보정 후 공간에서 다시 고른 값을 "
            "쓴다.\n\n"
            + ("이 세 확률은 **극한기상 전용 경사부스팅(GBM)**이 낸다(2026-09-26). "
               "신경망 헤드보다 F1 이 크게 높고(서빙 판정선 기준 한파 "
               "0.552→0.654), 무엇보다 "
               "**기온에 대한 단조성이 구조적으로 보장된다** — 기온이 오르면 한파 "
               "확률이 반드시 내려가도록 모델에 제약을 걸었다. 신경망에서는 이 성질이 "
               "학습 초기값에 따라 뒤집히곤 했고, 세 차례 처방이 모두 빗나갔던 "
               "문제다. 보정은 헤드마다 등온·베타·안 함 중 실측으로 더 나은 쪽을 "
               "골랐다."
               if _ex_gbm_on else
               "보정 전 원본 출력은 체계적으로 과신했다 — 희소 사건이 '항상 아님'"
               "으로 수렴하는 것을 막으려 학습 시 양성 표본에 준 가중치가 확률을 "
               "위로 밀어올린 결과이며, 검증셋에서 등온 회귀(isotonic regression)로 "
               "되돌렸다.")
            + " 상세 내용은 '성능 검증' 탭을 참조한다."
        )
        ...
        # 어느 키를 읽을지는 extreme_metrics_of() 한 곳에서 정한다 — 종전에는
        # 이 탭만 t=0.5 값을 읽어 '성능 검증' 탭과 두 배 차이가 났다.
        ...
        if _ex_gbm_on:
            st.info(
                "**아래는 종전 신경망 헤드의 판정선 경위 기록이다.** 현행 배포본은 "
                "극한기상을 전용 GBM 이 내며, 판정선을 그 모델의 보정 공간에서 "
                f"새로 골랐다(폭염 {_thr('heatwave', stn, ckpt):.3f} · "
                f"한파 {_thr('coldwave', stn, ckpt):.3f} · "
                f"황사 {_thr('dust', stn, ckpt):.3f}). 관측소별 예외는 적용하지 "
                "않는다 — 그 값들은 이전 모델의 확률 공간에서 검증한 것이라 근거가 "
                "없다. 아래 기록은 그 절차가 무엇이었고 무엇을 배웠는지를 남긴 것이다."
            )
        st.caption(
            "판정 임계값이 0.5가 아닌 이유: 사건이 드물어 0.5가 최선이 아닐 수 "
            "있으므로, 검증셋을 둘로 나누어 재보정을 시도한다(절차는 '성능 검증' "
            "탭 참조). 전체 관측소를 합산하면 폭염·한파는 재보정 이득이 통계적으로 "
            "유의하지 않아(순이득 각각 0.004·0.005 — 채택 기준 0.01 미달) 원본 "
            ...
            "+0.0689로 기준을 넘어 전용 임계값(0.33)을 채택했다(2026-08-19) — "
            "**다만 이 예외도 현행 모델에서는 적용되지 않는다**(아래 참조).  \n"
            "널 캘리브레이션(관측소별 무사건 구간 확률의 μ+z·σ로 임계값 후보를 "
            "재매개변수화하는 방식)으로 5건이 추가 채택됐다 — 폭염 수원·청주, "
            "한파 강릉·서울·인천. 후보 8건 중 3건은 제외했다: 인천 폭염·수원 "
            "한파는 임계값이 확률 보정 곡선의 평탄 구간에 떨어져(원본 공간의 "
            "이득이 판정이 실제로 이뤄지는 보정 후 공간에서 재현된다는 근거가 "
            "없다), 광주 한파는 그 관측소 F1은 올랐으나(0.000→0.098) 임계값이 "
            "낮아 오탐이 쏟아지면서 합산 한파 F1을 0.500→0.486으로 떨어뜨려 "
            "제외했다. 채택한 5건의 합산 효과는 폭염 0.814→0.817, 한파 "
            "0.500→0.500이었다. **다만 2026-09-07 승격한 현행 모델에는 전주 "
            "한파를 포함해 관측소별 예외가 하나도 적용되지 않는다** — 그 "
            "값들은 이전 모델의 확률 공간에서 "
            "검증해 고른 것이라 손실 구성이 바뀐 지금 모델에서는 근거가 없다"
            "(그대로 적용하면 서울 한파 판정선이 0.0105로 붕괴한다). 아래 "
            "목록은 재검증하면 되살릴 수 있는 후보로 남겨둔 것이다. "
            "전주 한파는 두 방법이 다른 값을 제시했으나(그리드서치 0.33 vs 널 "
            "캘리브레이션 0.2032) 기존 검증값을 우선해 유지했다(2026-08-31)."
        )
        st.caption(
...
with tab_perf:
    ...
    if val_temp is not None:
        ...
        # 기온 기준선도 기온을 낸 모델에서 읽는다 — 두 체크포인트가 같은
        # 분할이라 지금은 값이 같지만, 출처를 섞어 두면 나중에 한쪽만 바뀌었을
        # 때 조용히 어긋난다.
        _naive_temp   = (temp_ckpt or ckpt).get("val_temp_naive_mae")
        ...
        if _naive_precip and val_precip > _naive_precip:
            ...
            st.warning(
                ...
                f"크다. 이 사실을 은폐하지 않고 그대로 표시한다"
                + (f" — 화면에 나가는 값(확률 게이팅 후)은 "
                   f"{ckpt['val_precip_mae_served']:.4f}mm 로 격차가 "
                   f"{(ckpt['val_precip_mae_served'] - _naive_precip) / _naive_precip:.1%}"
                   f" 다.\n\n" if ckpt.get("val_precip_mae_served") is not None
                   else ".\n\n")
                ...
                f"참조한다.\n\n"
                f"구간별로 쪼개면 격차의 출처가 하나로 좁혀진다(2026-09-26 "
                f"현행 배포본 재측정, 서빙 후처리 적용 기준). **1mm 이상 전 "
                f"구간에서는 모델이 기준선을 이긴다** — 1~5mm·5~20mm·20mm 이상 "
                f"세 구간의 기여가 모두 음수(개선)다. 지는 이유는 표본의 93.4%인 "
                f"무강수 구간에서 출력이 정확한 0을 내지 못해 표본마다 평균 "
                f"0.038mm를 깔기 때문이며, 이 구간 하나가 만드는 손해(+0.0353mm)가 "
                f"전체 격차(+0.0093mm)의 3.8배다. 나머지 구간의 이득이 그 대부분을 "
                f"상쇄하고 남은 것이 현재 격차다. 확률 게이팅이 이 무강수 구간 "
                f"문제를 크게 줄였고(후처리 전 격차 28.0% → 후처리 후 5.7%), "
                f"그래도 남은 손해의 출처는 여전히 무강수 구간이다. "
                f"위 수치는 후처리 전 모델 출력 기준이고, 화면에 실제로 "
                f"나가는 값은 강수 확률이 "
                f"{_gate():.0%} 미만이면 "
                f"0으로 처리하는 게이팅을 거친 뒤다(2026-08-29 도입 — 그 이전에는 "
                f"0.2mm 미만을 반올림하는 방식이었고, 지금은 head_rain 이 없는 "
                f"구버전 체크포인트 폴백으로만 남아 있다).\n\n"
    ...
    # 서빙 판정선 기준 지표를 먼저 쓴다(2026-09-07). 체크포인트의
    # `extreme_metrics` 는 학습이 t=0.5 로 잰 값이라 화면의 판정과 다른
    # 동작점을 가리킨다 — 확률 눈금이 이동한 모델에서는 그 차이가 크다
    # (신경망 세대의 한파 F1 은 t=0.5 에서 0.209, 서빙 판정선에서 0.552였다 —
    # 현행 극한 GBM 은 t=0.5 에서 0.643, 서빙 판정선 0.320 에서 0.654다).
    # 이 파일의 목적이 "화면에 나가는 판정과 표의 값이 어긋나지 않게 하는
    # 것"이므로 서빙 기준을 표에 올린다. 없으면(구버전 체크포인트) 종전대로
    # t=0.5 값을 쓰고, 캡션이 그 사실을 밝힌다.
    _em, _em_is_served = extreme_metrics_of(ckpt)
    if _em:
        st.markdown("---")
        ...
        st.markdown("\n".join(_rows))
        # `t=0.5` 대조값은 체크포인트에서 읽는다 — 상수로 적어 두면 모델을
        # 바꿀 때마다 사람이 옮겨 적어야 하고, 실제로 신경망 시절 값(0.209)이
        # GBM 배포 후에도 남아 있었다(2026-09-26 수정).
        _raw_f1 = ((ckpt.get("extreme_metrics") or {}).get("coldwave") or {}).get("f1")
        if _em_is_served:
            st.caption(
                ...
                "기준으로 재면 값이 달라진다"
                + (f"(현행 배포본의 한파 F1은 {_raw_f1:.3f})"
                   if _raw_f1 is not None else "")
                + ". 확률 눈금은 손실 구성에 따라 이동하므로, 0.5는 "
                "모델마다 서로 다른 동작점을 뜻한다."
            )
        else:
            st.caption(
        ...
        if _dust_m:
            st.caption(
                ...
                + (f"(서빙 판정선 {_dust_m['threshold']:.0%} 기준)"
                   if _em_is_served else "(보정 전 원본 확률 0.5 기준)")
    ...
    if _cal:
        ...
        st.caption(
            ...
            "빈도와 일치하는가'는 별개의 문제다. 아래 표가 그것을 잰다."
        )
        if _ex_src:
            st.caption(
                "**극한기상 확률은 전용 경사부스팅(GBM)이 낸다(2026-09-26).** "
                "종전 신경망은 희소 사건이 '항상 아님'으로 수렴하는 것을 막으려 "
                "양성 표본에 준 가중치(pos_weight) 때문에 확률을 체계적으로 "
                "위로 밀어올렸고 — 2026-08-16 당시 한파를 60~70%로 표시하던 구간의 "
                "실제 빈도가 27% 였다 — 보정 없이는 액면대로 읽을 수 없었다. GBM 은 "
                "그 가중치를 쓰지 않아 **보정 전 확률이 이미 상당히 맞는다**"
                "(보정 전 ECE 한파 0.009 · 폭염 0.031)."
            )
            st.caption(
                "그래도 보정은 한다. 검증셋을 보정용·평가용으로 나누어 보정용에서만 "
                "곡선을 적합하고 평가용에서 채점하되, **후보에 '보정 안 함'을 "
                "포함한다** — 원본이 이미 잘 맞으면 곡선이 오히려 나쁠 수 있기 "
                "때문이다. 현재 배포본은 폭염·한파가 베타(beta), 황사가 등온 "
                "회귀(isotonic regression)를 골랐다. **다만 보정 품질 자체는 "
                "종전 신경망보다 나쁘다**(보정 후 ECE 폭염 0.010 vs 0.002 · 한파 "
                "0.008 vs 0.001). 판정 성능(F1)을 크게 얻고 이 지표를 소폭 내준 "
                "거래이며, 양쪽을 모두 표시한다."
            )
        else:
            st.caption(
                "측정 결과 네 헤드 모두 체계적으로 과신했다 — 2026-08-16 당시 "
                "한파를 60~70%로 표시하던 구간의 실제 발생 빈도는 27%에 불과했다. "
                "원인은 결함이 아니라 설계상의 대가다. 한파처럼 양성이 드문 사건은 "
                "모델이 '항상 아님'으로 답하는 자명해로 수렴하기 쉬워, 학습 시 "
                "양성 표본의 손실에 가중치(pos_weight)를 준다. 이것이 재현율을 "
                "확보하는 대신 확률을 위로 밀어올린다."
            )
            st.caption(
                "재학습 없이 사후 보정으로 되돌렸다. 검증셋을 보정용·평가용으로 "
                "나누어 보정용에서만 곡선을 적합하고, 평가용에서 개선을 확인한 "
                "헤드만 채택했다. 보정 방법은 헤드마다 실측으로 고른다."
            )
        st.caption(
            "⚠️ **다만 강수 헤드의 보정 곡선은 적합·보관만 하고 서빙에는 적용하지 "
            "않는다.** 강수량은 확률과 강도의 곱으로 산출되는데, 그 곱셈 구조가 "
            "보정 전 확률과 맞물려 학습됐기 때문이다(`predict.py` 강수 게이팅 "
            "주석 참조). 따라서 '출력값 추이' 탭에 표시되는 강수 확률과 그 "
            "판정 임계값은 **보정 전 원본 확률 공간의 값**이며, 아래 ECE 표에 "
        ...
        if os.path.exists(_cal_png):
            with st.expander("보정 전 신뢰도 곡선 보기"):
                ...
                st.caption(
                    ...
                    "전 상태이며, 화면의 폭염·한파·황사 확률은 이 어긋남을 "
                    "되돌린 값이다. 강수 확률만은 위에 적은 이유로 보정 전 "
                    "값을 그대로 표시하므로 이 곡선이 현재 상태다."
                )
    ...
    st.caption(
        ...
        "탐색한다 — 균일 격자(그리드서치)와 관측소별 무사건 분포 기반 후보"
        "(널 캘리브레이션) 두 탐색 공간을 쓰며, 목적함수는 둘 다 F1이다. "
        "③ 그 값을 한 번도 참조하지 않은 평가용에 적용하여 성능이 유지되는지 "
        "확인한다."
    )
    st.caption(
        ...
        f"공간의 값({_gate():.3f}, +12시간은 "
        f"{_gate(gbm_12h, 12):.3f})을 쓴다. "
        "아래 설명은 그 이전 세대의 절차 기록이다. "
        "**전역 기준값**에서는 강수만 이 검증을 통과해 재산출값(0.83)을 "
        "채택했고, 폭염·한파·황사는 통과하지 못해(순이득 각각 "
        "0.004·0.005·0.002 — 채택 기준 0.01 미달) 원본 확률 기준 0.5를 "
        "유지했다."
    )
    # 전역 상수의 길이를 그대로 쓰면 "적용 중"이 사실과 어긋난다 — 현행
    # 체크포인트에서 실제로 통과하는 조합만 센다(2026-09-07 점검에서 발견:
    # 수치예보 모델은 `station_thresh_overrides` 가 빈 목록이라 0건인데
    # 화면은 6건이 적용 중이라고 적고 있었다).
    _n_ovr = sum(1 for (_s, _e) in STATION_EVENT_THRESH_OVERRIDES
                 if _station_override_allowed(_e, _s, ckpt))
    st.caption(
        f"**관측소별 예외는 별도로 검증해 채택하며, 현행 배포본에서 실제로 "
        f"적용 중인 것은 {_n_ovr}건이다"
        + ("" if _n_ovr else f"(검증 이력이 있는 후보 "
                             f"{len(STATION_EVENT_THRESH_OVERRIDES)}건은 이전 "
                             f"모델의 확률 공간에서 고른 값이라 비활성이다)")
        + "** — 전역값은 12개 관측소를 합산해 고른 값이라, 공식 판정 "
        "기준이나 사건 빈도가 다른 관측소에서는 최선이 아닐 수 있다. 각 예외는 "
        "그 관측소 표본에서 같은 보정용/평가용 분리로 순이득 0.01 이상을 "
        "확인한 것만 채택했고, 판정이 실제로 이뤄지는 확률 보정 후 공간에서 "
        "임계값이 보존되는지도 함께 확인했다. 목록과 근거는 '극한 기상' 탭 "
        "설명을 참조한다."
    )
    ...
    if acc["cum_n"] == 0:
        ...
        # **강수 적중률은 단독으로 읽으면 안 된다(2026-09-27 추가).** 이 숫자는
        # 모델이 아니라 날씨에 따라 움직인다 — 온라인 기록 실측으로 건조한
        # 구간 99.5%, 비 온 구간 65.2% 가 나왔는데 후자의 F1 은 검증값과
        # 사실상 같았다. 저장소 규약(양성이 희박하면 accuracy 대신 F1)을
        # 정작 이 화면이 어기고 있었다. 기저율과 F1 을 함께 밝힌다.
        _w = acc.get("cum_wet") or {}
        _npos = _w.get("n_pos", 0)
        if _npos == 0:
            st.info(
                f"**위 강수 적중률({acc['cum_precip']:.0%})은 아직 모델의 강수 "
                f"판별력을 재지 않는다** — 누적 {acc['cum_n']}건 중 실제로 비가 온 "
                "표본이 **0건**이라, 무강수를 맞힌 비율일 뿐이다. 비가 오는 구간이 "
                "쌓이면 아래에 정밀도·재현율·F1 이 함께 표시된다."
            )
        else:
            # 셋 다 None 일 수 있다 — 비가 왔는데 강수를 한 번도 예측하지
            # 않으면 정밀도의 분모가 0이다(실측 로그에 실제로 있는 경우다).
            # f-string 에 그대로 꽂으면 화면이 죽으므로 여기서 문자열로 만든다.
            def _pct(v):
                return f"{v:.1%}" if v is not None else "—(해당 예측 없음)"
            _f1 = _w.get("f1")
            _f1s = f"{_f1:.3f}" if _f1 is not None else "—"
            st.info(
                f"**강수는 적중률이 아니라 아래 값으로 판단한다.** 누적 "
                f"{acc['cum_n']}건 중 실제로 비가 온 표본은 **{_npos}건**"
                f"({_npos / acc['cum_n']:.1%})뿐이라, 적중률은 대부분 "
                "'무강수를 맞힌 비율'이다 — 건조한 기간에는 저절로 올라가고 비가 "
                "오면 내려간다.\n\n"
                f"- 정밀도(precision) **{_pct(_w['precision'])}** · 재현율(recall) "
                f"**{_pct(_w['recall'])}** · **F1 {_f1s}**"
                + (f"\n- 검증셋 기준값은 F1 {ckpt['val_precip_wet_f1']:.3f}"
                   f"(정밀도 {ckpt['val_precip_wet_precision']:.1%} · 재현율 "
                   f"{ckpt['val_precip_wet_recall']:.1%})이다."
                   if ckpt.get("val_precip_wet_f1") is not None else "")
                + f"\n\n표본이 {_npos}건뿐이므로 검증값과의 차이를 성능 변화로 "
                "읽지 않는다 — 방향이 크게 어긋나는지만 본다."
            )
...
with tab_model:
    ...
    if temp_ckpt is not None:
        ...
        # 극한기상까지 GBM 으로 옮긴 뒤에는 **다중과제 신경망이 화면에 내는
        # 값이 하나도 없다**(2026-09-26). 종전 문구는 "극한기상 확률은 아래
        # 구조의 다중과제 신경망이 낸다"고 적어 두었는데, 그 상태에서 극한을
        # GBM 으로 옮기면 화면이 사실과 다른 출처를 가리킨다 — 판정선을
        # 옮길 때와 같은 유형의 드리프트다. 출처 키로 분기해 막는다.
        _ex_on = ckpt.get("extreme_source") is not None
        st.info(
            "**이 화면은 모델 "
            + ("세" if _ex_on else ("세" if _gbm_on else "두"))
            + " 개의 출력을 함께 보여준다.** 기온과 그 90% 예측구간은 "
            f"**기온 전용 모델**(검증 MAE {_t_solo:.2f}°C)이"
            + (", 강수는 **강수 전용 경사부스팅(GBM)**이" if _gbm_on else "")
            + (", 극한기상 확률은 **극한기상 전용 GBM**(단조 제약)이 낸다.\n\n"
               if _ex_on else ", 극한기상 확률은 아래 구조의 "
               "**다중과제 신경망**이 낸다.\n\n")
            ...
            + ("강수를 신경망이 아닌 모델이 내는 이유는 다르다 — 같은 입력을 받는 표형 GBM 이 "
               ...
               if _gbm_on else "")
            + ("**그 결과 아래 구조가 화면에 내보내는 값은 하나도 없다.** 축 배분과 융합 "
               "수식은 여전히 이번 입력으로 실제 계산한 값이지만, 그것을 받아 쓰는 출력이 "
               "배포에서 빠졌다 — Re축(공간 보간장)과 Im축(시간 경향)도 쓰이지 않는다. "
               "이 저장소가 무엇을 만들었고 왜 산출물에서 빠졌는지를 남기기 위해 서술을 "
               "지우지 않고 둔다."
               if _ex_on else
    ...
    if gw:
        _ex_on = ckpt.get("extreme_source") is not None
        st.markdown(
            "#### 축 기여도 — 다중과제 신경망의 3축 배분"
            if _ex_on else "#### 축 기여도 — 이번 출력값에 대한 3축 배분")
        st.caption(
            "3축의 기여도는 고정값이 아니라 출력마다 입력을 근거로 재계산된다. "
            "소형 신경망이 세 값을 산출하고 소프트맥스(softmax, 합이 1이 되도록 "
            "정규화하는 함수)를 거치므로 합은 항상 100%다. 아래 '3축 융합 수식'의 "
            "`w`가 이 값이다."
            + ("\n\n**이 배분은 위 출력값을 만들지 않는다** — 이번 입력으로 실제 "
               "계산한 값이지만, 이 신경망의 출력은 현재 화면 어디에도 쓰이지 "
               "않는다(위 안내 참고)." if _ex_on else "")
        )
    ...
    # Z축 차원은 체크포인트에서 계산한다 — 상수로 적어두면 입력을 늘려도
    # 화면만 옛 값에 머문다(2026-09-23 발견: 수치예보 14차원이 편입돼
    # num_features 가 28이 된 뒤에도 "수치 14개"로 남아 있었다).
    _nf_total = ckpt.get("num_features", 14)
    _nwp_dim = (nwp_feature_dim(ckpt.get("nwp_feature_set") or "full14")
                if ckpt.get("use_nwp") else 0)
    _z_obs_dim = _nf_total - _nwp_dim
    _z_cell = (f"이번 시각의 관측값 — 기온·강수·습도·풍속·풍향·기압·위도·경도·"
               f"연중 시각 등 수치 {_z_obs_dim}개")
    if _nwp_dim:
        _z_cell += (f", 그리고 수치예보(NWP) 예보값에서 산출한 부가 특징 "
                    f"{_nwp_dim}개(합계 {_nf_total}개)")
    st.markdown(
        "| 축 | 질문 | 입력 데이터 |\n"
        "|---|---|---|\n"
        f"| Z축(현재 상태) | 이 관측소는 현재 어떤 상태인가 | {_z_cell} |\n"
    ...
    if _nwp_dim:
        _nwp_note = (
            f"수치예보(NWP) 부가 특징 {_nwp_dim}개는 2026-09-07 배포부터 Z축에 "
            "편입됐다 — 유효 시각 24시간 전에 발표된 예보를 쓰므로 리드타임이 "
            "보존되고 시간 누수가 없다. 지상 관측만 쓰던 이전 세대보다 기온·강수 "
            "지표가 모두 개선됐다(수치는 '성능 검증' 탭 참조). 자료 출처는 화면 "
            "하단에 표기한다."
        )
        if ckpt.get("extreme_nwp_neutral_dims"):
            _nwp_note += (
                f" 다만 극한기상 헤드에는 이 {ckpt['extreme_nwp_neutral_dims']}개 "
                "차원을 중립화해 전달한다 — 그대로 넣으면 한파 헤드가 기온에 "
                "거꾸로 반응했다."
            )
        st.caption(_nwp_note)
    st.caption(
    ...
    with st.expander("이 수식의 출처 — 원 논문과의 관계"):
        ...
        st.markdown(
            ...
            "| 수식의 출력 | 검색 순위 점수 | 6개 예측 헤드의 공통 입력 |\n"
        )
        st.markdown(
    ...
    if ckpt.get("signed_head_input"):
        st.caption(
            ...
            "없다. 그 결과 이전 모델은 한겨울에 폭염 확률을 올렸다 — 2026-08-17 "
            "측정 당시 실제 −15°C 이하 관측의 85.7%에서 폭염 확률이 판정선을 "
            "넘었다. Z축 벡터는 제곱을 "
            "거치지 않아 부호가 살아 있으므로, 이를 함께 넣어 두 극단을 구분하게 "
            "했다(그 조치 직후 재측정 85.7% → 0.08%. 두 수치 모두 당시 모델 "
            "기준이며, 현행 배포본의 값은 '출력값 추이' 탭에 적는다). "
            "기온 헤드가 절대값 대신 변화량을 학습하는 "
    ...
    st.markdown(
        ...
        "장치다.\n"
        f"- 마지막으로 강수 확률이 "
        f"{_gate(_lead=ckpt.get('lead_hours', 6)):.0%} "
        "미만이면 강수량을 0으로 반올림한다(리드타임마다 확률 분포가 달라 "
