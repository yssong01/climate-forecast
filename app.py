...
with tab_perf:
    ...
    if _cal:
        ...
        st.caption(
            ...
            "체계적으로 과신했다 — 2026-08-16 당시 한파를 60~70%로 표시하던 "
            "구간의 실제 발생 빈도는 27%에 불과했다. 현행 배포본에서도 보정 전 "
            "최대 오차(MCE)는 한파 0.81·황사 0.80으로 여전히 크다."
        )
        st.caption(
            "원인은 결함이 아니라 설계상의 대가다. 한파처럼 양성이 드문 사건은 "
            "모델이 '항상 아님'으로 답하는 자명해로 수렴하기 쉬워, 학습 시 양성 "
            "표본의 손실에 가중치(pos_weight)를 준다. 이것이 재현율을 확보하는 "
            "대신 확률을 위로 밀어올린다. 2026-08-17 모델에서는 보정 오차의 크기 "
            "순서가 이 가중치의 크기 순서와 정확히 일치했다(폭염 3.1배 → 한파 "
            "13.6배 → 강수 15.6배 → 황사 36.0배) — 다만 그 뒤 특보 비운영기간을 "
            "확정 음성으로 채우면서 라벨 구성이 바뀌어 가중치도 달라졌고, 현행 "
            "배포본에서는 이 순서를 다시 확인하지 않았다."
        )
        st.caption(
            "재학습 없이 사후 보정으로 되돌렸다. 검증셋을 보정용·평가용으로 "
            "나누어 보정용에서만 곡선을 적합하고, 평가용에서 개선을 확인한 헤드만 "
            "채택했다(임계값 산출과 동일한 절차). 보정 방법은 헤드마다 실측으로 "
            "고른다 — 현재 배포본은 강수·폭염이 베타(beta) 보정, 한파·황사가 "
            "등온 회귀(isotonic regression)다."
        )
        st.caption(
