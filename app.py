...
with tab_extreme:
    ...
    if not any(ev.get(k) is not None for k in ("heatwave", "coldwave", "dust")):
        ...
        st.caption(
            ...
            "재매개변수화하는 방식)으로 6건이 추가 채택됐다 — 폭염 수원·청주, "
            "한파 강릉·서울·인천·광주(순이득 +0.016~+0.092, 원본 확률 공간 기준). "
            "후보는 8건이었으나 인천 폭염·수원 한파 2건은 임계값이 확률 보정 "
            "곡선의 평탄 구간에 떨어져 제외했다 — 원본 공간에서 잰 이득이 판정이 "
            "실제로 이뤄지는 보정 후 공간에서 재현된다는 근거가 없기 때문이다. "
