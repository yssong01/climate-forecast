...
with tab_extreme:
    ...
    if not any(ev.get(k) is not None for k in ("heatwave", "coldwave", "dust")):
        ...
        st.caption(
            "⚠️ 황사 확률은 이 화면에서 뺐습니다. 재보정 후에도 정밀도가 0.115"
            "(황사라고 판정한 것 중 88.5%가 오탐)에 그쳐 실용 수준이 못 됩니다. "
        ...
        st.caption(
            ...
            "폭염은 이 재보정으로 얻는 이득이 통계적으로 유의하지 않아(순이득 "
            "0.005) 기본값 0.5를 그대로 씁니다 — 재보정한다고 항상 더 나아지는 "
            "건 아닙니다."
        )
        st.caption(
