...
with tab_perf:
    ...
    _em_served = ckpt.get("extreme_metrics_served") or {}
    _em = _em_served or (ckpt.get("extreme_metrics") or {})
    _em_is_served = bool(_em_served)
    if _em:
        st.markdown("---")
    ...
    st.caption(
        ...
        "**[2026-09-07 갱신] 현행 배포본은 판정선을 원본 확률 공간이 아니라 "
        "확률 보정 후 공간에서 직접 고른다**(폭염 0.402 · 한파 0.330 · "
        "황사 0.109 · 강수 0.262). 손실 구성이 바뀌면 확률 눈금 자체가 "
        "이동하므로, 원본 공간의 상수를 곡선으로 옮기는 것만으로는 판정이 "
        "보존되지 않기 때문이다. 아래 설명은 그 이전 세대의 절차 기록이다. "
