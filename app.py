...
with tab_extreme:
    ...
    if not any(ev.get(k) is not None for k in ("heatwave", "coldwave", "dust")):
        ...
        st.caption(
            ...
            "0.500→0.500이었다. **다만 2026-09-07 승격한 현행 모델에는 이 "
            "예외들이 적용되지 않는다** — 그 값들은 이전 모델의 확률 공간에서 "
...
with tab_perf:
    ...
    st.caption(
        f"**관측소별 예외는 별도로 검증해 현재 {len(STATION_EVENT_THRESH_OVERRIDES)}건이 "
        "적용 중이다** — 전역값은 12개 관측소를 합산해 고른 값이라, 공식 판정 "
