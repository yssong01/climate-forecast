...
with tab_extreme:
    ...
    if not any(ev.get(k) is not None for k in ("heatwave", "coldwave", "dust")):
        ...
        st.caption(
            ...
            "10.7%(황사로 판정한 것 중 89.3%가 오탐)에 그쳐 실용 수준에 미달한다. "
        ...
        st.caption(
            ...
            "표시되는 값이 0.5보다 낮은 것은 확률 보정을 거치면서 임계값도 같은 "
            "곡선으로 함께 옮겼기 때문이며, 판정 결과는 동일하다.  \n"
            "이전 모델에서는 부산 폭염에만 전용 임계값(0.33)을 적용했다. 현행 "
            "모델에서 재검증하니 부산 전용값의 순이득이 −0.0033으로 오히려 전역 "
...
with tab_perf:
    ...
    if _em:
        ...
        if os.path.exists(_calib_path):
            ...
            st.caption(
                ...
                "춘천 40건인 반면 광주 4건·부산 2건·제주 1건에 그친다 — 시간 "
    ...
    if _cal:
        ...
        for _k, _ko, _e0, _e1, _f in (
            ("heatwave", "🔥 폭염", 0.0338, 0.0048, "0.8144 → 0.8144"),
            ("coldwave", "🥶 한파", 0.0517, 0.0031, "0.4492 → 0.4471"),
        ):
            if _k in _cal:
