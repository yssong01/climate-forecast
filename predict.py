...
# 14차원 입력(연중 시각 sin/cos 추가) + 한파 헤드 디커플링 반영 체크포인트 기준).
#
# 왜 또 다시 쟀는가: 한파 헤드만 디커플링(트렁크 동결·헤드만 미세조정)했더니
# 그 헤드의 확률 분포 자체가 바뀌었다. 이전에 고른 임계값(0.89)은 디커플링 전
# 분포에 맞춘 값이라 그대로 쓰면 안 된다 — 헤드가 바뀔 때마다 임계값도 그
# 헤드 기준으로 다시 골라야 한다.
#
# 채택 기준은 종전과 동일: 보정용/평가용 분할에서 t=0.5 대비 순이득이 0.01을
# 넘는 것만 재보정값을 쓴다(threshold_validation.py 자체 기준).
#   강수    — 순이득 +0.101 → 재보정값(0.83) 채택, F1 0.510 (t=0.5는 0.410)
#   폭염    — 순이득 +0.007 → 기준 미달, t=0.5 유지 (F1 0.812, 이미 근접 최적)
#   한파    — 순이득 +0.001 → 기준 미달, t=0.5 유지. 디커플링 전에는 재보정
#            (0.89)이 유의미했는데(순이득 +0.095), 디커플링으로 헤드 분포가
#            바뀌면서 그 값의 근거가 사라졌다 — 추측이 아니라 이번에 다시
#            잰 결과다.
#   황사    — 순이득 +0.021 → 재보정값(0.86) 채택. 다만 재보정해도 정밀도가
#            낮아(precision_breakdown 참고) 화면에서는 제외했다. 헤드 자체는
#            유지한다(내부적으로는 여전히 계산됨) — EXTREME_EVENT_THRESH 에
#            값을 남겨 predict()가 확률은 계속 반환하되, app.py 가 표시하지
#            않는다.
