...
# 날짜 단위 그룹 분할로 재학습한 체크포인트 기준).
#
# 왜 다시 쟀는가: 종전 체크포인트는 시각 단위 무작위 분할로 학습·검증했는데,
# 폭염·한파·황사 라벨이 (관측소, 날짜) 단위라 판정 가능한 검증 표본의 100%가
# 학습셋과 같은 날짜를 공유했다(split_leakage_check.py 실측). 즉 모델이 그
# 관측소·그 날짜의 정답을 이미 본 상태에서 채점받고 있었다. 날짜를 통째로
# 한쪽에 배정하는 그룹 분할로 바꾸자 F1 이 크게 떨어졌다 — 성능이 나빠진
# 게 아니라 부풀림이 걷힌 것이다(README '정직한 한계' 참고).
#
...
#   한파    — 순이득 +0.095 → 재보정값(0.89) 채택, F1 0.475 (t=0.5는 0.379)
#   황사    — 재보정해도 정밀도 0.115(오탐 88.5%)에 그쳐 화면에서 제외했다.
#            헤드 자체는 유지한다(내부적으로는 여전히 계산됨) — EXTREME_EVENT_THRESH
#            에 값을 남겨 predict()가 확률은 계속 반환하되, app.py 가 표시하지
#            않는다.
