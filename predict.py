...
# 로 찾은 8건. 방법: 무사건(공식 라벨 음성) 구간 확률의 μ_null+z·σ_null 을
# 임계값 후보로 두고, z 도 보정용 절반의 F1로만 골라 평가용 절반에서
# 채점했다(threshold_validation.py 와 동일 CALIB_SEED=1234 분할, 순이득
# 0.01 이상만 채택). 부산 폭염에서는 이 방식이 기존 F1-그리드서치와 F1이
# 완전히 일치(0.5735)해 방법 자체의 타당성을 뒷받침했다. 전주 한파(146)는
# 이미 그리드서치로 검증된 값(아래)이 있어 건드리지 않는다 — 두 방법이
# 다른 값(0.33 vs 0.2032)을 제시했는데, 기존 값이 이미 프로덕션에서
# 검증된 것이라 대체 없이 유지한다.
STATION_EVENT_THRESH_OVERRIDES: dict = {
    ("146", "coldwave"): 0.33,
    ("112", "heatwave"): 0.6565,  # 인천, 순이득 +0.0240
    ("119", "heatwave"): 0.6781,  # 수원, 순이득 +0.0163
    ("131", "heatwave"): 0.7142,  # 청주, 순이득 +0.0157
    ("105", "coldwave"): 0.4167,  # 강릉, 순이득 +0.0186
    ("108", "coldwave"): 0.6129,  # 서울, 순이득 +0.0170
    ("112", "coldwave"): 0.5269,  # 인천, 순이득 +0.0162
    ("119", "coldwave"): 0.6000,  # 수원, 순이득 +0.0502
    ("156", "coldwave"): 0.1749,  # 광주, 순이득 +0.0921 (t=0.5 에서 F1=0 이던 구간)
}
