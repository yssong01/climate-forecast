...
#
# **평탄 구간 배제(2026-08-31).** 여기 담는 값은 원본(raw) 공간이고
# event_threshold() 가 보정 곡선으로 옮겨 판정하는데, 등온 회귀의 평탄
# 구간에 임계값이 떨어지면 판정이 보존되지 않는다(CLAUDE.md 12절, 실제
# F1 0.4566→0.4484 사고). 후보 8건을 곡선에 대조한 결과 인천 폭염
# (raw 0.6565)과 수원 한파(raw 0.6000) 두 건이 τ±0.01 구간에서 보정폭
# 0.0 인 완전 평탄 구간에 떨어져 **채택하지 않았다** — raw 공간에서 잰
# 순이득(+0.0240·+0.0502)이 서빙 공간에서 재현된다는 근거가 없기 때문이다.
# 나머지 6건은 보정폭 4.5e-03~5.9e-02 로 판정이 보존된다.
# 전주 한파(146)는 이미 그리드서치로 검증된 값이 있어 건드리지 않는다.
STATION_EVENT_THRESH_OVERRIDES: dict = {
    ("146", "coldwave"): 0.33,
    ("131", "heatwave"): 0.7142,  # 청주, 순이득 +0.0157 (보정폭 4.7e-02)
    ("119", "heatwave"): 0.6781,  # 수원, 순이득 +0.0163 (보정폭 4.5e-03)
    ("105", "coldwave"): 0.4167,  # 강릉, 순이득 +0.0186 (보정폭 2.7e-02)
    ("108", "coldwave"): 0.6129,  # 서울, 순이득 +0.0170 (보정폭 7.0e-03)
    ("112", "coldwave"): 0.5269,  # 인천, 순이득 +0.0162 (보정폭 5.9e-02)
    ("156", "coldwave"): 0.1749,  # 광주, 순이득 +0.0921 (보정폭 1.3e-02)
}
