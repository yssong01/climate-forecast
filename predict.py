...
# ① 세 헤드 모두 F1 이 크게 오른다(한파 — 실험 채점 0.4607→0.6077 ·
# 서빙 채점 0.552→0.654. 두 채점을 섞어 인용하지 말 것). ② `monotonic_cst` 가
...
#
# **2026-09-07 재선정(수치예보 모델 승격 후).** 규칙 9 — 헤드를 다시
# 조정하면 판정선도 그 시점 표본에서 다시 고른다. 두 리드타임을 모두
# 재학습했으므로 이 게이트도 재검증했다(보정용에서 F1 최대 τ 를 고르고
# 평가용에서만 채점, CALIB_SEED=1234 로 날짜 분할):
#   +6h  현행 0.850 평가용 F1 0.5747 · 재선정 0.865 는 0.5708 (순이득
#        −0.0039) → **현행 유지.** 확률 분포가 바뀌었는데도 종전 값이
#        여전히 최적 근처라는 것이 실측으로 확인됐다.
#   +12h 현행 0.750 평가용 F1 0.5410 · 재선정 **0.805** 는 0.5555
#        (순이득 **+0.0145**, 채택 기준 0.01 초과) → **교체.** 종전 값은
#        수치예보 이전 모델의 rain_prob 분포에서 고른 것이라, 분포가
#        오른쪽으로 이동한 새 모델에서는 너무 낮아 오탐이 늘고 있었다.
PRECIP_PROB_GATE_BY_LEAD = {6: 0.85, 12: 0.805}
...
def load_model(checkpoint_path: str = CHECKPOINT, device: str = DEVICE):
    ...
    model = TriCHEFPipeline(
        ...
        extreme_temp_neutral_idx=ckpt.get("extreme_temp_neutral_idx") or None,
        fusion=ckpt.get("fusion", "modulus"),
