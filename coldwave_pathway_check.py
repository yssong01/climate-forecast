...
# 0.64~1.08 · 0.39~0.64 다 — **겹치는 구간이 없다.** `compact6 / 44` 는
# 상관이 −0.14 로 통과 방향인데 계절의존 0.68 이라 불안정한 통과다 —
# 종전 게이트는 이걸 잡지 못했다.
#
# **[2026-09-07 정정] 계절의존도는 원인이 아니라 동반 지표다.** 처음에는
# "헤드가 계절에 의존해서 기온 순위가 잡음으로 정해진다"고 해석하고,
# 부호 경로에서 계절 성분(연중 시각 12·13)을 중립화하면 나아질 것이라
# 예측했다. **반증됐다** — 계절의존은 0.47→0.44 로 내려갔지만 기온진폭이
# 0.83→0.62 로 함께 내려갔고 상관은 −0.40→+0.13 으로 나빠져 PASS→WARN 이
# 됐다. 비율이 좋아져도 실질은 나빠질 수 있다.
#
# 그래서 이 판정은 **보수적 추가 신호로만** 쓴다 — corr 판정과 함께 나쁜
# 쪽을 채택하므로 낮은 계절의존이 나쁜 상관을 구제하지는 못한다(위
# 계절중립화 사례가 정확히 그 경우다: 계절의존 PASS·상관 WARN → WARN).
# 참고로 7개 표본에서는 **기온진폭**이 더 깨끗하게 가른다(PASS 0.83~0.84
# vs 나머지 0.39~0.64) — 다만 표본이 적어 판정 기준으로 삼지 않는다.
#
