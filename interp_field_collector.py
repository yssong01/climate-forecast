...
**강수 채널을 시도했다가 기각한 경위(2026-08-31 통계 재검증으로 사유 정정).**
data_expansion_probe.py 의 절제 실험에서 "이웃 현재값"(강수 포함) 단계의
증분이 ΔAUC +0.0254 로 국지 경향 다음으로 컸는데, 그 신호가 이 4채널
격자에는 전혀 반영되지 않고 있었다 — 강수는 채널 스펙에 없었다. 이 공백을
메우려 5채널로 늘려 대조 실험(동일 SEED=42, group split, RE_CHANNELS 만
변경)을 돌렸다. 점추정치만 보고 "기온만 개선, 나머지 4개 전부 악화"로
기각했으나, bootstrap_ci_compare.py(날짜 993개 블록 단위 짝지은 부트스트랩,
B=2,000)로 재검증하니 95% CI가 0을 포함하지 않는 것은 기온 개선
(−0.0575°C)과 황사 악화(−0.0245 F1) 둘뿐이었다 — 강수·폭염·한파는 점추정치가
나빠 보였을 뿐 재표본 노이즈 범위 안이라 "악화됐다"고 주장할 근거가 없었다.
**결론은 그대로 기각**이다(황사 헤드는 정확도가 아니라 Re축 게이트 학습을
견인하는 구조적 역할 때문에 유지 중이라 그 악화가 실제라는 점이 여전히
걸림돌) — 다만 원래 사유("이득 1개 대 손해 4개")는 과장이었다. 상세 경위는
CLAUDE.md 5절·rejected-re-channel-precip 메모 참고. train.py 의 RE_CHANNELS
기본값은 4로 되돌렸다 — n_bands=5 경로는 재현 가능하도록 코드에 남겨둔다.
"""
import os
