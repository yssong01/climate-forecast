...
**한파 F1은 `+6h`와 `+12h`가 지금 다른 세대의 헤드를 쓴다(2026-08-19 갱신).** `+6h`는 아래 [중첩 전이학습 승격](#nested-transfer-promotion)을 거쳐 F1 0.499인 반면, `+12h`는 이 개선 이전(리드타임 실험 당시)의 한파 헤드를 그대로 써 F1 0.453이다 — 같은 화면에서 리드타임에 따라 한파 성능 격차가 두드러진다. `+12h`에도 같은 커리큘럼을 적용할지는 아직 결정하지 않았다.

`+12h` 후보는 배포 게이트 3종(계절 오탐·단조성·관측소 사각지대)을 확인했다. 단조성은 깨끗이 통과했으나, **계절 오탐이 5.41%로 WARN 판정을 받았다**(배포본 0.06% 대비 높음, 차단 기준 10%는 넘지 않음). 원인은 한파 헤드가 25~30°C 구간에서 오발화하는 것으로, 과거 문제였던 "한겨울 폭염 오탐"(부호 표현 부재)과는 다른, 더 약한 형태의 잔여 오탐이다. FAIL이 아니라는 점과 절대 성능이 여전히 실용적이라는 점을 근거로 WARN 상태로 채택했다 — 화면에도 이 판단 근거를 캡션으로 노출한다.

체크포인트는 `checkpoints/numerical_trichef_12h.pt`로 분리해 배포한다(+6h 배포 경로는 건드리지 않음). 확률 보정·임계값 재선정도 이 체크포인트에서 독립적으로 다시 수행했다(강수 t=0.182, 폭염 t=0.368, 한파 t=0.239, 황사 t=0.124).
