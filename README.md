...
**그럼에도 서빙 동작을 지금 바꾸지 않는다.** 모델은 학습 중에도 결측을 0.5로 본 상태로 학습했으므로, 0.5는 물리적 중립값은 아니지만 **모델이 배운 결측 표식**이다. 서빙에서만 대체값을 바꾸면 학습·서빙 불일치가 생긴다. 올바른 해법은 결측 여부를 알리는 마스크 채널을 입력에 추가하는 것이며, 이는 입력 차원 변경이라 재학습과 구버전 호환 처리가 함께 필요하다([작업 규약 7](CLAUDE.md)). 다음 재학습 때 함께 처리할 항목으로 남긴다.
