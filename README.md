...
3. **보조축이 통째로 죽음** — 이웃(Re축)·과거(Im축)를 `SUCCESS_LIVE`로만 받고 있어, 연결이 막히면 실측이 저장소에 있는데도 0/11·0/3이 되어 중립값으로 채워졌다. 실측 여부와 최신 여부를 분리해 판정하도록 고쳤다(같은 시각 스냅숏 조건은 유지 — IDW는 동시각 공간 분포를 전제하므로). 재현 실측 **11/11 · 3/3 복원**.
...
**재배포와 신선도를 분리했다(2026-08-20).** 이전에는 main에 커밋했고, Streamlit Cloud가 그 push를 감지해 자동 재배포하면서 신규 데이터를 반영했다 — 신선도를 올리려면 재배포도 그만큼 자주 겪어야 하는 구조였다(재배포마다 캐시 소실·수십 초 끊김). 그래서 2026-08-19에는 6시간→2시간으로만 절충했다.
