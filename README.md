...
황사는 본 표와 대시보드에서 제외했다. 정밀도가 10.7%에 불과해 황사로 판정한 결과 중 89.3%가 오탐이다. PM10 농도는 국외 장거리 수송의 영향이 지배적이나 본 모델의 입력은 국내 12개 지점의 기상 관측값에 한정되어, 구조적으로 판별 단서가 부족한 것으로 판단된다.
...
| 지표 | 현행 배포 | A: 강수 헤드 부호 표현 | B: 헤드 드롭아웃 0.1 |
|---|---|---|---|
| 기온 MAE | **1.2966°C** | 1.4104°C | 1.3120°C |
| 강수(기준선 대비) | **−9.9%** | −20.7% | −12.7% |
| 폭염 F1 | **0.814** | 0.801 | 0.809 |
| 한파 F1 | 0.449 | 0.415 | **0.472** |
| 계절 오탐 최악값 | **0.08%** | — | **38.8%(FAIL)** |
...
남은 가능성으로 **한파 헤드에만 드롭아웃을 적용하는 방안**이 있다. 폭염 헤드의 부호 경로를 건드리지 않으면서 희소 헤드의 정규화 효과만 취하는 구성이며, 아직 검증하지 않았다.
...
| 한파 | 0.0513 | **0.0031** | 0.461 | 0.4492 → 0.4471 |
| *강수* | *0.1719* | *0.0017* | *0.573* | *0.4965 → 0.4941* |
| *황사* | *0.1196* | *0.0006* | *0.785* | *0.1866* |
