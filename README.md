...
**데이터 갱신 원칙 — 저장소가 원본이다.** Streamlit Cloud의 컨테이너 파일시스템은 휘발성이므로 앱이 기록한 파일은 재시작 시 소실된다. 이에 갱신 주체를 앱이 아닌 저장소로 설정한다. `.github/workflows/refresh-data.yml`이 2시간마다 `refresh_deploy_data.py`를 실행하여 ① 최근 관측 창을 갱신하고 ② 적중률 로그의 대기 항목을 실측값과 대조한 후, 결과를 main 브랜치에 재커밋한다. Streamlit Cloud가 이 push를 감지해 자동 재배포하면서 신규 데이터가 반영된다. 이 워크플로 역시 `KMA_API_KEY` 시크릿이 필요하다(회당 약 24건, 일일 약 288건 — 일일 호출량은 격차 보충 방식이라 주기와 무관하게 일정하다). 주기는 2026-08-19에 6시간에서 2시간으로 줄였다. 이 창이 [외부 장애](#outage-20260819) 시의 실측 폴백으로도 쓰이게 되면서, 창이 묵으면 폴백 품질이 그대로 나빠지기 때문이다.
...
| 단계 | 자동화 여부 | 근거 |
|---|---|---|
| 신규 관측 수집 | ✅ **이미 자동화됨** | `.github/workflows/refresh-data.yml`(2시간 주기) + `refresh_deploy_data.py` |
