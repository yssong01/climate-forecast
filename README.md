...
| 헤드 | 층화 축 | 층화 평가 커버리지 | 목표 |
|---|---|---|---|
| 기온 | 관측소(12개) | 0.8992 | 0.90 |
| 강수 | 예측 강수확률 구간(4개) | 0.8996 | 0.90 |
...
결과는 체크포인트의 `conformal_interval` 키에 저장했다(백업: `numerical_trichef_before_conformal.pt`). **현재 화면(Streamlit)에는 아직 연결하지 않았다** — 계산·검증까지만 이번에 마쳤고, 표시 UI 연결은 남은 작업이다.
...
**중첩 극한사건 전이학습 자체의 유효성은 여전히 미확인 상태다** — 재시도하려면 사전학습 체크포인트를 다시 만들고, 이번엔 `CHECKPOINT_PATH`가 실제로 반영됐는지 확인한 뒤 미세조정을 돌려야 한다.
