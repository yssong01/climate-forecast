...
> 같은 날, 연중 주기 특징(day-of-year feature)을 추가한 재학습(이하 A안)도 시도했다. 한파 헤드(head)가 기온에 반대로 반응하는 오류가 발견되어 원상복구했다 — "시도했다가 원복한 시험" 항목을 참조한다.
...
| A안 직후(14차원, 디커플링 이전) | **0.357** | **0.587** | **0.923** | **0.984** | **0.991** |

근본 원인은 아직 규명하지 못했다. 트렁크(trunk)·헤드 간 상호작용, 신규 축과 기존 축의 정규화(normalization) 상호작용을 후보로 검토 중이다. 원인 규명 전까지는 A안을 제외하고 이전 체크포인트(12차원)로 배포를 유지한다. 기온 4.2% 개선 효과는 손실이나, 한파 헤드가 온도에 반대로 반응하는 상태로 배포할 수는 없었다.
...
- (2026-08-16) A안(연중 시각 특징 추가) 재학습 후 한파 헤드가 기온에 반대 방향으로 반응했다 — 다른 조건을 고정하고 기온만 15°C에서 33°C로 상승시키면 한파 확률이 0.14에서 0.97로 상승한다. 배포 전 F1·MAE만 검토했다면 발견하지 못했을 문제다(집계 지표는 정상 범위였다). `coldwave_sensitivity_check.py`로 물리적으로 자명한 방향성(기온 상승 시 한파확률 하강)을 직접 검증하고서야 발견했다. 상세 경위는 [결과 요약의 "시도했다가 원복한 시험"](#시도했다가-원복한-시험--연중-주기-특징-추가-a안-2026-08-16)을 참조한다. 교훈: 집계 성능 지표(F1·MAE)가 정상이어도, 물리적으로 자명한 단조성(monotonicity)은 헤드 또는 입력 차원 변경 시마다 별도로 검증해야 한다.
