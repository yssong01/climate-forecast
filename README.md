...
이 탭 안에 리드타임 토글이 있다. `LEAD_HOURS`를 환경변수로 빼(`train.py`) 12/24/48시간을 각각 재학습해 성능 저하 곡선을 실측한 결과, +12시간은 절대오차 악화가 완만해(기온 MAE 1.30→1.66°C, 한파 F1 0.458→0.453으로 사실상 그대로) 2차 산출값으로 채택했다. +24/48시간은 한파·황사 F1이 눈에 띄게 무너져(0.45→0.39→0.37) 채택하지 않았다.
