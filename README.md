...
### 요약(Summary) — 그룹 분할(group split) 검증 기준, 2026-08-16

| 항목 | 값 | 평가 |
|---|---|---|
| 기온 평균절대오차(MAE) | 1.36°C | 퍼시스턴스(persistence) 대비 **+59.5%** |
| 강수 MAE | 0.179mm | 상시 무강수 예측 대비 **17.9% 미달**(기준선 미도달) |
...
- 기온 1.36°C — 퍼시스턴스(T(t+6)≈T(t)) 대비 **+59.5% 개선**.
- 강수 0.179mm — 상시 무강수 예측 대비 **17.9% 미달**. 기준선에 미치지 못하는 지표도 은폐하지 않고 그대로 제시한다.
...
- **폭염: 부산만 유의하게 낮다 — 판정 기준 자체가 상이하다.** 나머지 11개 관측소는 재현율 82~91%, 정밀도 74~85%로 군집을 이루나, 부산(관측소 코드 159)만 재현율 71%, 정밀도 47%로 이탈한다. 원인 분석을 위해 오탐(모델이 폭염으로 예측했으나 공식 미판정)일 때의 기온을 확인한 결과, 부산은 오탐 시 평균 기온이 28.65°C로 부산 자신의 실제 폭염일 평균(27.38°C)보다 오히려 높았다. 대구는 반대로 오탐 시 기온(27.46°C)이 실제 폭염일 평균(28.58°C)보다 낮다 — 대구는 판정 경계 부근에서 혼동이 발생하는 통상적 패턴이며, 부산은 최고 기온일조차 공식적으로 폭염 미판정될 만큼 판정 기준 자체가 다르다. 모델은 관측소 구분 없이 공통 기준으로 학습되었으므로, 부산에서만 체계적으로 낮은 성능을 보인다.
  ...
  재학습 없이 부산의 판정 임계값만 별도로 산출했다(`station_threshold_check.py`, 절차는 위와 동일한 보정용/평가용(calibration/test) 분리). 전체 관측소를 통합해 산출한 임계값(0.5)은 부산에 부적합했으나, 부산 단독으로 산출한 0.33은 평가용 표본에서 순이득 +0.039로 유의했다(정밀도는 0.448→0.443으로 거의 변화 없음, 재현율은 0.688→0.848로 개선). 임계값을 ±0.02 조정해도 F1이 안정적이었다. 현재 부산에만 이 값을 적용한다(`predict.py`의 `STATION_EVENT_THRESH_OVERRIDES`) — 다른 관측소는 검증 없이 개별 조정하면 과적합(overfitting)이므로 전역값을 유지한다.
...
이 플롯은 `calibration_plot_diagnose.py`로 재생성한다. 자동 갱신되지 않으므로 재학습 또는 체크포인트 교체 시 수동으로 실행해야 한다.
...
| 강수 MAE | 0.1786mm | 상시 무강수 예측: 0.1515mm | **17.9% 미달** |

강수 지표는 이전 결과(무작위 분할 기준 0.8% 미달)보다 격차가 확대되었으나, 이 역시 검증 방식 변경의 영향이 일부 포함되어 있다 — 인접 시각 표본 간 자기상관(autocorrelation)이 무작위 분할에서는 일부 누수되고 있었다. 헤드 4종(강수·폭염·한파·황사)이 하나의 공유 표현(shared representation)을 두고 경쟁하는 구조적 원인은 해소되지 않아, 본 지표는 여전히 개선 과제로 남아 있다.

동적 게이팅(dynamic gating)이 학습한 축 배분은 공간보간장(Re) 41.3%, 시간경향벡터(Im) 22.5%, 수치 센서(Z) 36.2%다.
...
| *v9(12차원, 배포 중)* | *통과* | *1.3551°C* | *−17.9%* | *0.813* | *0.465* |
...
원인 규명 전까지 A안을 제외하고 이전 체크포인트(12차원)로 배포를 유지한다. 기온 4.2% 개선 효과는 손실이나, 절반의 확률로 한파 헤드가 뒤집히는 구성을 배포할 수는 없다. 재시도한다면 계절 특징과 한파 헤드의 결합을 구조적으로 제약하는 방법(헤드별 입력 마스킹 등)을 먼저 검토해야 한다.
...
위 한파 수치는 한파 헤드만 별도로 디커플링(트렁크 동결, 헤드만 미세조정, Kang et al. 2020 방식)한 결과다. 디커플링 이전 F1 0.384에서 0.465로 개선되었으며, 기온·강수·폭염·황사 지표는 완전히 동일하게 유지되었다(트렁크 동결이 다른 헤드에 영향을 주지 않음을 함께 확인했다).

임계값은 검증셋 절반(보정용)에서 산출하고 나머지 절반(평가용)에서 검증하는 절차를 따른다. 강수·황사는 재산출값(각각 t=0.81, 순이득 +0.099·+0.028)을 채택했다. 폭염·한파는 순이득이 각각 +0.005·+0.009로 채택 기준(0.01)에 미달하여 기본값 0.5를 유지한다.

황사는 본 표와 대시보드에서 제외했다. 재산출해도 정밀도가 11.7%에 불과해, 황사로 판정한 결과 중 88.3%가 오탐이다. PM10 농도는 국외 장거리 수송의 영향이 지배적이나 본 모델의 입력은 국내 12개 지점의 기상 관측값에 한정되어, 구조적으로 판별 단서가 부족한 것으로 판단된다. 헤드 자체는 유지되어 계산은 계속되나 화면에는 표시하지 않는다.
...
|---|---|---|---|---|
| 폭염 | 0.0388 | **0.0043** | 0.221 | 0.8122 → 0.8119 |
| 한파 | 0.0446 | **0.0034** | 0.372 | 0.4629 → 0.4629 |
| 강수 | 0.1915 | 0.0014 | 0.558 | 0.5001 → 0.5001 |
| 황사 | 0.2936 | 0.0008 | 0.804 | 0.1776 → 0.1776 |

ECE(Expected Calibration Error)는 구간별 "표시 확률 − 실제 빈도"의 차이를 표본 수로 가중평균한 값이며, MCE(Maximum Calibration Error)는 그 최댓값이다. 한파의 ECE가 낮아 보이는 것은 표본 64,130개 중 50,771개가 0~0.1 구간에 몰려 있기 때문이며, 화면에 실제로 노출되는 중·고확률 구간은 MCE 0.372가 보여주듯 크게 어긋나 있었다 — 60~70%로 표시되던 구간의 실제 발생 빈도는 27%였다.
...
재학습 없이 등온 회귀(isotonic regression)로 되돌렸다(`probability_calibration_fit.py`). 절차는 임계값 선정과 동일하다 — 검증셋을 보정용/평가용으로 분리하여 보정용에서만 곡선을 적합하고, 평가용에서 개선이 확인된 헤드만 채택한다. **순위를 바꾸지 않는 단조 변환이므로 분류 성능은 보존된다**(임계값도 같은 곡선으로 함께 옮긴다). 폭염의 F1이 0.0003 하락한 것은 등온 회귀의 평탄 구간에서 임계값 부근의 동점 처리가 달라진 결과이며, 그 외 세 헤드는 완전히 동일하다.
...
|---|---|---|---|
| **Z**(현재 상태) | 이 지점은 현재 어떤 상태인가 | 기온·강수·습도·풍속·풍향(sin/cos)·기압·강수형태·시각(sin/cos)·위도·경도 = 12차원 | — (기준축) |
...
가장 큰 실패 항목이며, 은폐하지 않고 화면에 표시한다. 2026-08-11 재학습에서 격차를 +0.8%까지 축소한 바 있다(이전 −19.3%). 강수는 대부분의 시각이 0mm이므로, 별도 계산 없이 "항상 0mm"로 응답해도 MAE가 매우 낮게 산출된다. 현재 모델의 강수 MAE는 이 기준선을 넘어서지 못했다.

원인은 구조적으로 규명되었다. 극한기상 헤드가 3개(폭염→한파→황사)로 증가하면서, 헤드들이 하나의 공유 표현(`s`)을 두고 경쟁하여 허들 구조의 무강수 억제력이 약화되었다(기준선 대비 −16.0%에서 −25.6%로 악화). `EXTREME_BCE_WEIGHT`를 1.0에서 0.3으로 낮춰 −19.3%까지 완화했으며, 추론 후처리로 0.2mm 미만을 0으로 반올림해 잔여 손실을 축소했다. 신규 헤드 추가 시마다 강수 MAE를 재확인하는 것이 본 프로젝트의 규칙이 된 배경이다.

강수는 MAE보다 "강수 발생 여부의 적중 여부"가 실용적 가치에 가까우며, 이는 대시보드 '성능 검증' 탭의 강수 적중률(0.1mm 기준 이진 판정 일치)로 별도 제시한다.

> 기준선 수치는 학습 시 계산은 하나 체크포인트에 저장하지 않던 버전이므로, 현재 체크포인트로는 화면에서 직접 비교하지 못한다. `train.py`에 `val_temp_naive_mae`·`val_precip_naive_mae` 저장을 추가했으므로 다음 재학습부터는 대시보드에 증감으로 자동 표시된다.
...
`test_phase36.py` T1에서 두 값의 최대 차이가 1.19e-07로 확인되었다. θ가 축 자체의 표본별 정보를 담지 못하고, 게이트 가중치를 극좌표로 재표현한 값에 불과하다는 의미다. 검색 과제(원 논문)에서는 축 임베딩의 노름(norm)이 자유로워 θ가 의미를 가지나, 단위 정규화를 사용하는 본 회귀 구현에서는 해당 개념이 성립하지 않는다. 표본별 축 정보는 축 간 코사인(`cos(Re,Im)` 등, 표준편차 0.109)에만 남아 있어, 상태에는 이를 대신 포함했다.
...
**후속 — 시간 분할 교차검증(2026-08-16).** 그룹 분할과 별개로, 데이터가 13년으로 확장되면서 처음 가능해진 시간 분할(과거로 학습, 미래로 검증)도 컷오프(cutoff) 세 지점으로 검증했다. 배치 1024, 학습률 1e-3, 마스킹 ON은 모든 조건에서 동일하다.
...
| 재학습 실행 | ⚠️ **로컬 GPU 필요** | GitHub Actions 무료 러너는 GPU를 제공하지 않는다. 전체 학습에 15~35분(GPU 기준)이 소요된다 |
| 체크포인트 검증·배포 | ✅ 자동화 가능 | 재학습 후 검증 MAE·F1이 기존보다 저하되면 배포를 차단하는 게이트를 CI에 추가할 수 있다 |

**권장 운영 방식**: 관측 수집만 상시 자동으로 실행하고(현재 구현됨), 분기마다 ① 포털에서 라벨 CSV를 수동 다운로드하고 ② 로컬 GPU에서 `train.py`를 실행하고 ③ 검증 지표가 기존 체크포인트보다 저하되지 않았는지 확인하고 ④ 체크포인트를 커밋한다. ③은 이미 규칙으로 강제되어 있다 — 재학습 전 반드시 백업하고(`numerical_trichef_vN_<사유>.pt`), 신규 헤드 추가 시마다 강수 MAE를 재확인한다.
