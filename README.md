...
이 워커를 세우는 과정에서 도구 쪽 함정을 하나 만났다. **`npx --yes wrangler`처럼 버전을 고정하지 않고 받은 wrangler 4.x로 등록한 시크릿이 실제 서빙 런타임에 연결되지 않았다.** `wrangler secret put`·`wrangler versions secret put` 어느 쪽도 매번 "Success"를 출력했고 `wrangler versions view`의 시크릿 목록에도 나타났지만, 워커 코드에서 `env.GITHUB_PAT`을 읽으면 빈 문자열이어서 GitHub이 401 Bad credentials로 거부했다. 값 대신 길이와 SHA-256 지문만 로그로 남기는 임시 디버그 코드를 넣고서야 확인했다(길이 0, 빈 문자열의 해시값과 일치). 배포 순서를 바꾸거나 버전을 프로덕션 트래픽에 100%로 명시 승격해도 재현됐다. `cloudflare-worker/package.json`에 wrangler 3.x를 고정하고 `npm install`로 로컬 설치하면 `npx wrangler`가 그 버전을 우선 사용해 해결된다 — 3.x에서는 시크릿이 즉시 연결되고 코드만 재배포해도 재등록 없이 유지된다. 최종 검증은 사람이 개입하지 않은 순수 자동 Cron 실행(2026-08-20 07:45 UTC)이 GitHub Actions에 `workflow_dispatch` 실행을 성공적으로 생성하는 것으로 확인했다. 교훈: CLI가 "성공"이라고 말하는 것과 그 설정이 런타임에 실제로 반영되는 것은 별개이며, 이 간극은 런타임에서 직접 값을 확인해야만 드러난다.
