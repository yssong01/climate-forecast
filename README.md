...
### 기각한 시험 — 한파 헤드 중첩 극한사건 전이학습(2026-08-18)

한파는 F1 0.458로 네 헤드 중 가장 낮고, 원인은 표본 자체(관측소당 사건 8~40건, 공식 라벨 기간 2020년~)다. 공식 라벨 기간은 포털 제공 범위라 못 늘리지만, 원시 기온은 13.6년치가 있다. Jacques-Dumas et al.(arXiv:2103.09743)의 중첩 극한사건 전이학습 — 더 극한적인 사건은 덜 극한적인 사건의 부분집합이라는 성질을 이용해 표본이 많은 "약한" 사건으로 먼저 학습한 뒤 진짜 희소 사건으로 옮겨가는 방법 — 을 시험했다.

`coldwave_nested_pretrain.py`가 학습셋 기온 하위 백분위 3단계(20/10/5%)로 head_coldwave를 트렁크 동결 상태에서 순차 사전학습하고, `head_decouple_finetune.py`로 공식 라벨 미세조정을 이어 붙인다. 약한 라벨은 사전학습에만 쓰고 평가에는 전혀 쓰지 않는다(공식 라벨 vs 임계값 근사를 섞으면 안 된다는 원칙 — 폭염 F1 사고 재발 방지, [지표 분리 측정](#metrics-and-breakdown) 참고).

커리큘럼 자체는 의도대로 작동했다 — 공식 라벨을 한 번도 보지 않은 상태로 참고용 F1이 단계마다 단조 상승했다(0.239 → 0.348 → 0.413). 그러나 최종 미세조정 결과는 일반 디커플링(F1 0.4576)과 구별되지 않았다.

| 실행 | 최선 에폭 F1 | 30에폭 평균 F1 |
|---|---|---|
| 일반 디커플링(프로덕션) | 0.4576 | (전체 곡선 미보존, 단일값) |
| 중첩 전이학습 1회차 | 0.4630 | **0.4580** |
| 중첩 전이학습 2회차(재현 확인) | 0.4640 | **0.4580** |

처음엔 순이득(+0.0054)처럼 보였으나, 독립적으로 두 번 재현한 결과 30에폭 전체 평균 F1이 두 실행 모두 **소수점 넷째 자리까지 완전히 일치**(0.4580)했고 프로덕션의 단일 보고값과도 사실상 같았다. **"최선 에폭 고르기"가 만든 착시였다** — 학습 곡선 자체가 에폭마다 0.452~0.464 사이를 오갔고, 관찰된 순이득은 그 잡음 구간 상단을 우연히 뽑은 것이었다. head_coldwave가 4,161개 파라미터짜리 작은 MLP라 30에폭 공식 라벨 미세조정만으로도 사전학습 유무와 무관하게 같은 지점에 수렴하는 것으로 추정한다.

코드는 `coldwave_nested_pretrain.py`로 재현 가능하게 남기되, 근거 없이 재시도하지 않는다 — 재시도하려면 이 결론이 성립하지 않는 조건(더 큰 헤드, 더 적은 미세조정 에폭, 다른 학습률 등)을 먼저 특정해야 한다.
