...
차트 아래 표는 기온·강수 출력값을 +6시간과 +12시간에 대해 동시에 보여준다. 각 값에는 현재 대비 변화량과 검증셋 평균절대오차(MAE)를 함께 적고, 현재 대비 상승은 빨간색·하강은 초록색으로 표시한다(2026-08-20 개편 — 종전에는 라디오 버튼으로 한 리드타임만 골라 보여줬으나, 두 값을 한 화면에서 바로 비교할 수 있도록 토글을 없앴다).
...
API·인증키·호출량 모두 정상이며 배포 환경의 발신 IP 경로만 막혔다는 결론이다(호출량 84/3,000건, `weather_collector.py`는 2026-08-16 이후 무변경). 기상청 API허브에 문의했고 답변을 기다리는 중이다.
