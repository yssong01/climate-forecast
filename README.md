...
### 5. 외부 장애 — 배포 환경 발신 IP만 차단(2026-08-19)
...
부수적으로 관측소별 예외 임계값이 불필요해졌다. 이전 배포본에서는 부산 폭염만 정밀도가 낮아(47%) 전용 임계값 0.33을 적용했으나, 현행 모델에서 재검증한 결과 부산 전용값의 순이득이 **−0.0033**으로 오히려 전역 0.5보다 나빴다(부산 F1 0.574). 검증을 통과하지 못한 예외는 두지 않는다는 규약에 따라 제거했다. (2026-08-19 갱신: 전주 한파에 새 예외가 채택됐다 — [아래 참조](#station-override-jeonju).)
...
`+12h` 부체크포인트는 이 개선을 아직 받지 않았다 — 위 [+12시간 2차 산출값](#12h-secondary) 절 참고.
