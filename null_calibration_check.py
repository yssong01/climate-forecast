...
(μ_null, σ_null)로 임계값을 **재매개변수화**해, 균일 격자 대신 그 관측소의
무사건 분포에 맞춘 성긴 후보만 탐색한다(cross-project-technique-survey 메모
2단계 항목 5, Risk_Prediction calibration.py fit_null() 이식).

**주의 — 이 구현은 F1 을 직접 최적화한다(2026-08-31 정정).** 최초 서술은
"F1 을 직접 최적화하지 않으므로 표본 노이즈에 덜 취약할 수 있다"였으나,
아래 `z_scored` 는 보정용 절반의 **F1 이 가장 높은 z** 를 고른다 — 즉 기존
그리드서치와 목적함수가 같고 탐색 공간만 다르다. 따라서 "두 방법의 F1 이
일치했으므로 방법의 타당성이 뒷받침된다"는 식의 추론은 성립하지 않는다
(같은 표본에서 같은 목적함수를 최대화했으니 일치가 당연하다). 원래
fit_null() 처럼 목표 오탐율(FAR)에서 z 를 역산해 F1 을 아예 보지 않는
구성으로 바꿔야 그 주장이 가능하다 — 그 변경은 하지 않았다.
...
CALIB_SEED(1234)로 헤드별 표본을 절반씩 재분할한다. 후보 그리드(Z_GRID)
중 **보정용 절반**에서 F1이 가장 높은 z만 고르고 **평가용 절반**에서만
채점한다 — μ_null·σ_null 도 보정용 절반의 무사건 표본에서만 계산한다.
고르는 표본과 채점하는 표본이 분리돼 있어 순환 논리는 없다(이 점은
2026-08-31 점검에서 재확인됨).

**이 스크립트가 내는 순이득은 원본(raw) 확률 공간의 값이다.** 서빙은 보정
후 공간에서 판정하므로(predict.event_threshold), 채택 전에 임계값이 등온
회귀의 평탄 구간에 떨어지지 않는지 반드시 확인해야 한다 — 평탄 구간이면
여기서 잰 이득이 서빙에서 재현되지 않는다(CLAUDE.md 12절). 실제로
2026-08-31 후보 8건 중 2건이 이 검사에서 탈락했다.
