...
def main():
  ...
  3. README.md 와 app.py 의 수치·서술을 새 모델 기준으로 갱신한다:
       ...
       · 임계값·예외에 관한 설명
  3-1. **어떤 출력의 출처(모델)가 바뀌었다면, 화면에서 옛 출처 이름을
     직접 grep 한다** — 예: 극한기상을 GBM 으로 옮겼다면
       grep -n "다중과제 신경망\\|신경망 헤드" app.py
     지표는 체크포인트 조회로 따라오지만 **"무엇이 이 값을 낸다"는 서술은
     사람이 고쳐야 한다.** 2026-09-26 에 강수만 고치고 극한기상 쪽을
     남겨 '모델 구조' 탭이 "극한기상 확률은 다중과제 신경망이 낸다"고
     설명하는 상태로 배포됐다. 같은 유형(화면이 서빙과 다른 것을 가리킴)이
     그날 세 번 났다 — 판정선 상수·지표 키·출처 서술.
