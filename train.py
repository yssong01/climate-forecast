...
#
# 환경변수로 뺀 이유(2026-08-18) — 리드타임별 성능 저하 곡선을 재기 위함이다.
# "+6시간 산출값이 언제까지 쓸모 있는가"는 코드를 고치지 않고는 답할 수 없는
# 질문이었는데, 이 값 하나만 바꿔 대조 실행하면 곡선이 나온다. 배포 기본값은
# 6으로 유지한다 — 체크포인트에 lead_hours 가 저장되고 predict.py 가 그것을
# 읽으므로, 대조 실행은 CHECKPOINT_PATH 를 분리하기만 하면 배포와 섞이지 않는다.
# 주의: 퍼시스턴스 기준선도 리드타임에 따라 달라지므로(T(t+L)≈T(t)), 서로 다른
# L 의 MAE 를 직접 비교하지 말고 각자의 기준선 대비 개선폭으로 비교할 것.
...
# (2026-08-30, 2026-08-31 통계 재검증으로 사유 정정) — data_expansion_probe.py
# 의 단순 프로브는 "이웃 현재값" 단계(강수 포함)에서 ΔAUC +0.0254 를 냈지만,
# 전체 파이프라인 재학습(동일 SEED=42, group split)의 점추정치만 보면 기온
# MAE 만 개선되고 강수·폭염·한파·황사 4개가 전부 악화된 것처럼 보였다.
# bootstrap_ci_compare.py(날짜 블록 짝지은 부트스트랩, B=2,000)로 재검증하니
# 95% CI가 0을 포함하지 않는 것은 기온 개선(1.3219→1.2644°C)과 황사 악화
# (0.176→0.151 F1) 둘뿐이었다 — 강수·폭염·한파는 재표본 노이즈 범위 안이라
# "악화"라는 근거가 없었다. 결론은 그대로 기각(황사 헤드는 Re축 게이트 학습을
# 견인하는 구조적 역할 때문에 그 악화가 여전히 걸림돌)이지만 "이득 1개 대
